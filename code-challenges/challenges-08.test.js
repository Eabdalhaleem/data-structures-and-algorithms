...
const sortByChildren = (charArray) => {
  // Solution code here...
  characterData.sort((e,i) => {
    if (e.children.length > i.children.length) {
      return 1;

    } else if (e.children.length == i.children.length) {
      return i.house.length-e.house.length
...
const containsW = (str) => {
  // Solution code here...
  let expressionregex=/w/g
  
  let result= str.match(expressionregex)?true:false 
  return result;
};
...
const isNum = (input) => {
  // Solution code here...
  let result;
  let expressionregex=/world/g
  input.match(expressionregex)? result= true : result= false
  return result
};
